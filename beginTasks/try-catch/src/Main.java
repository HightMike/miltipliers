...
public class Main {
    ...
    public static void moveRobot(RobotConnectionManager robotConnectionManager, int toX, int toY) {
        for (int i =0; i<3; i++) {
            try(RobotConnection connect = robotConnectionManager.getConnection()) { // try с ресурсами, при завершении ресурсы освобождаются
                connect.moveRobotTo(toX,toY); // после подключения идем в указанную точку
                i=3;
            }
            catch (RobotConnectionException e) { // ловим исключения при коннекте
                if (i==2) { // только после третьей попытки бросаем исключение
                    throw new RobotConnectionException(e.getMessage(), e);
                }
            }
            catch (Exception t) { // ловим все остальные
                throw t;
            }
        }
    }
